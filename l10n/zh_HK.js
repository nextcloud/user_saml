OC.L10N.register(
    ...
    "Service Provider Entity ID (optional)" : "服務提供者實體 ID（非強制的）",
    ...
    "Attribute to map the users to an existing LDAP user" : "用來將使用者對映至現有 LDAP 用戶的屬性",
    ...
    "Make sure to configure an administrative user that can access the instance via SSO. Logging-in with your regular account will not be possible anymore, unless you go directly to the URL {url}." : "請確保已設定一個可透過 SSO 存取實例的管理員用戶。除非您直接前往網址 {url}，否則將無法再使用一般帳戶登入。[cite: 21, 22]",
    "Provider {id}" : "提供者 {id}",
    "Could not create provider configuration" : "無法建立提供者設定 ",
    "Could not load provider settings" : "無法載入提供者設定 ",
    "Saved" : "已儲存",
    "Could not save configuration" : "無法保存配置",
    "Unknown error (status {code})" : "錯誤不詳（狀態碼 {code}）",
    "Unable to update configuration" : "無法更新配置",
    "Could not add identity provider" : "無法新增身分提供者",
    "Could not remove identity provider" : "無法移除身分提供者",
    "Single sign-on and SAML authentication settings" : "單一登入 (SSO) 及 SAML 驗證設定",
    ...
    "Environment var starting with HTTP_ are not allowed as HTTP headers are saved in these environment variables" : "不允許使用以 HTTP_ 開頭的環境變數，因為 HTTP 標頭會被儲存於這些環境變數內。",
