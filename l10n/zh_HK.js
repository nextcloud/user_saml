OC.L10N.register(
    ...
    "Only allow authentication if an account exists on some other backend. (e.g. LDAP)" : "僅在其他後端存在賬號時才允許身份驗證。 （例如 LDAP）",
    ...
    "Optional display name of the identity provider (default: \"SSO & SAML log in\")" : "可選的身份提供者的顯示名稱（默認：\"SSO及SAML登錄\"）",
    "Allow the use of multiple user back-ends (e.g. LDAP)" : "允許使用多個用戶後端（例如 LDAP）",
    "SSO & SAML authentication" : "SSO & SAML 認證",
    "Authenticate using single sign-on" : "使用單點登錄認證",
    "Using the SSO & SAML app of your Nextcloud you can make it easily possible to integrate your existing Single-Sign-On solution with Nextcloud. In addition, you can use the Nextcloud LDAP user provider to keep the convenience for users. (e.g. when sharing)\nThe following providers are supported and tested at the moment:\n\n* **SAML 2.0**\n\t* OneLogin\n\t* Shibboleth\n\t* Active Directory Federation Services (ADFS)\n\n* **Authentication via Environment Variable**\n\t* Kerberos (mod_auth_kerb)\n\t* Any other provider that authenticates using the environment variable\n\nWhile theoretically any other authentication provider implementing either one of those standards is compatible, we like to note that they are not part of any internal test matrix." : "使用 Nextcloud 的 SSO 和 SAML 應用程式，您可以輕鬆地將現有的單點登錄解決方案與 Nextcloud 集成。此外，您可以使用 Nextcloud LDAP 用戶提供程序為用戶（例如，共享時）提供便利。\n\n目前支持以下提供程序：\n\n*** SAML 2.0 **\n\t* OneLogin\n\t* Shibboleth\n\t* Active Directory聯合身份驗證服務（ADFS）\n\n* **通過環境變量驗證**\n\t* Kerberos（mod_auth_kerb）\n\t*使用環境變量進行身份驗證的任何其他提供程序\n\n從理論上講，實現上述標準之一的任何其他身份驗證提供程序都是兼容的，但我們要指出的是，它們不屬於任何內部測試矩陣。",
    "Open documentation" : "打開文檔",
    "Make sure to configure an administrative user that can access the instance via SSO. Logging-in with your regular %s account won't be possible anymore, unless you enabled \"%s\" or you go directly to the URL %s." : "請確保設置一個能通過SSO訪問實例的管理員用戶。您將無法使用您的普通 %s 賬號登錄，除非您啟用 \"%s\" 或直接訪問URL %s。",
    "Make sure to configure an administrative user that can access the instance via SSO. Logging-in with your regular %s account won't be possible anymore, unless you go directly to the URL %s." : "請確保設置一個能通過SSO訪問實例的管理員用戶。您將無法使用您的普通 %s 賬號登錄，除非您直接訪問URL %s。",
    ...
    "Show Service Provider settings…" : "顯示服務器供應商設置...",
    ...
    "Show optional Identity Provider settings…" : "顯示可選的身份提供者設置...",
