OC.L10N.register(
    ...
    "SAML authentication is not configured. Please ask your administrator to complete the SAML setup in the admin panel." : "SAML認証が設定されていません。管理者に依頼して、管理パネルでSAMLの設定を完了してください。",
    ...
    "Use normal Nextcloud login form" : "通常のNextcloudログインフォームを使用する",
    ...
    "Attribute to map the users MFA login status" : "ユーザーのMFAログイン状態をマップするための属性",
    "Attribute to map the users to an existing LDAP user" : "ユーザーを既存のLDAPユーザーにマッピングするための属性",
    "Group Mapping Prefix, default: %s" : "グループマッピング接頭辞、デフォルト：%s",
    "Reject members of these groups. This setting has precedence over required memberships." : "これらのグループのメンバーを拒否します。この設定は必須メンバーよりも優先されます。",
    "Group A, Group B, …" : "グループ A, グループ B, …",
    "Require membership in these groups, if any." : "もしあれば、これらのグループへの加入を義務付ける。",
    ...
    "Using the SSO & SAML app of your Nextcloud you can make it easily possible to integrate your existing Single-Sign-On solution with Nextcloud. In addition, you can use the Nextcloud LDAP user provider to keep the convenience for users. (e.g. when sharing)\nThe following providers are supported and tested at the moment:\n\n* **SAML 2.0**\n\t* OneLogin\n\t* Shibboleth\n\t* Active Directory Federation Services (ADFS)\n\t* Authentik\n\n* **Authentication via Environment Variable**\n\t* Kerberos (mod_auth_kerb)\n\t* Any other provider that authenticates using the environment variable\n\nWhile theoretically any other authentication provider implementing either one of those standards is compatible, we like to note that they are not part of any internal test matrix." : "Nextcloud の SSO & SAML アプリを使用すると、既存のシングルサインオンソリューションを Nextcloud と簡単に統合できます。さらに、Nextcloud の LDAP ユーザープロバイダーを使用することで、ユーザーの利便性（例: 共有時）を維持できます。 \n現在、以下のプロバイダーがサポートおよびテストされています。\n\n* **SAML 2.0**\n* OneLogin\n* Shibboleth\n* Active Directory Federation Services (ADFS)\n* Authentik\n\n* **環境変数による認証**\n* Kerberos (mod_auth_kerb)\n* 環境変数を使用して認証を行うその他のプロバイダー\n\n理論上は、これらいずれかの標準を実装している他の認証プロバイダーも互換性がありますが、それらは内部テストマトリックスには含まれていない点にご注意ください。",
    "Make sure to configure an administrative user that can access the instance via SSO. Logging-in with your regular account will not be possible anymore, unless you go directly to the URL {url}." : "SSO経由でインスタンスにアクセスできる管理者ユーザーを設定してください。URL{url}に直接アクセスしない限り、通常のアカウントでのログインはできなくなります。",
    "Provider {id}" : "プロバイダー {id}",
    "Could not create provider configuration" : "プロバイダーの設定を作成できませんでした",
    "Could not load provider settings" : "プロバイダーの設定を読み込めませんでした",
    "Saved" : "保存しました",
    "Could not save configuration" : "設定を保存できませんでした",
    "Unknown error (status {code})" : "不明なエラー（ステータス {code}）",
    "Unable to update configuration" : "設定を更新できません",
    "Could not add identity provider" : "IDプロバイダーを追加できませんでした",
    "Could not remove identity provider" : "IDプロバイダーを削除できませんでした",
    "Single sign-on and SAML authentication settings" : "シングルサインオンとSAML認証の設定",
    ...
    "Registered Providers" : "登録済みのプロバイダー",
    "Register new provider" : "プロバイダーの新規登録",
    "Remove {name}" : "{name} を削除",
    "Environment variable provider settings" : "環境変数プロバイダーの設定",
    ...
    "Environment var starting with HTTP_ are dangerous as HTTP headers are saved in these environment variables" : "HTTP_ で始まる環境変数は、HTTPヘッダーがこれらの環境変数に保存されるため危険です。",
    ...
    "Request parameters to pass-through to IdP (comma separated list)" : "IdPに渡すリクエストパラメータ（カンマ区切りリスト）",
    ...
    "User filtering" : "ユーザーフィルタリング",
    "If you want to optionally restrict user login depending on user data, configure it here." : "オプションでユーザデータに応じてユーザログインを制限する場合は、ここで設定します。",
    ...
    "Choose an authentication provider" : "認証プロバイダの選択",
    "Group Mapping Prefix, default: SAML_" : "グループマッピング接頭辞 デフォルト: SAML_"
},
"nplurals=1; plural=0;");
