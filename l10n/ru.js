OC.L10N.register(
    ...
    "Make sure to configure an administrative user that can access the instance via SSO. Logging-in with your regular %s account will not be possible anymore, unless you enabled \"%s\" or you go directly to the URL %s." : "Убедитесь, что у вас настроен пользователь с правами администратора, который может получить доступ к экземпляру через SSO. Вход в систему с вашей обычной учетной записью %s больше не будет возможен, пока вы не включите \"%s\" или не перейдёте непосредственно по URL-адресу %s.",
    "Make sure to configure an administrative user that can access the instance via SSO. Logging-in with your regular %s account will not be possible anymore, unless you go directly to the URL %s." : "Убедитесь, что у вас настроен пользователь с правами администратора, который может получить доступ к экземпляру через SSO. Вход в систему с вашей обычной учетной записью %s больше не будет возможен, пока вы не перейдёте непосредственно по URL-адресу %s.",
    ...
    "Show user filtering settings …" : "Показать настройки фильтрации пользователей …",
