OC.L10N.register(
    ...
    "Indicates whether the <samlp:AuthnRequest> messages sent by this SP will be signed. [Metadata of the SP will offer this info]" : "指出此 SP 傳送的 <samlp:AuthnRequest> 訊息是否會被簽署。[SP 的中介資料將會提供此資訊]",
    ...
    "Whether the metadata should be signed." : "中介資料是否應被簽署。",
    ...
    "Indicates a requirement for the <saml:Assertion> elements received by this SP to be signed. [Metadata of the SP will offer this info]" : "指出此 SP 接收的 <saml:Assertion> 元素需要簽署的要求。[SP 的中介資料將會提供此資訊]",
    ...
    "Use normal Nextcloud login form" : "使用一般 Nextcloud 登入表單",
    ...
    "Attribute to map the users to an existing LDAP user" : "用來將使用者對映至現有 LDAP 使用者的屬性",
    ...
    "Make sure to configure an administrative user that can access the instance via SSO. Logging-in with your regular account will not be possible anymore, unless you go directly to the URL {url}." : "請確保設定一個可以透過 SSO 存取站台的管理員使用者。您將無法使用您的普通帳號登入，除非您直接存取 URL {url}。",
    "Provider {id}" : "提供者 {id}",
    "Could not create provider configuration" : "無法建立提供者組態",
    "Could not load provider settings" : "無法載入提供者設定",
    "Saved" : "已儲存",
    "Could not save configuration" : "無法儲存組態",
    "Unknown error (status {code})" : "未知錯誤（狀態 {code}）",
    "Unable to update configuration" : "無法更新組態",
    "Could not add identity provider" : "無法新增身份驗證提供者",
    "Could not remove identity provider" : "無法移除身份驗證提供者",
    "Single sign-on and SAML authentication settings" : "單一登入與 SAML 驗證設定",
    ...
    "Request parameters to pass-through to IdP (comma separated list)" : "要傳遞給 IdP 的請求參數（逗號分隔清單）",
    ...
    "Metadata valid" : "中介資料有效",
    "Metadata invalid" : "中介資料無效",
    "Loading…" : "正在載入……",
    "Error" : "錯誤",
    "Please wait while you are redirected to the SSO server." : "請稍後，您會被重新導向至 SSO 伺服器。",
    "JavaScript is disabled in your browser. Please enable it to continue." : "您的瀏覽器已停用 JavaScript。請啟用以繼續。",
    "Access denied." : "拒絕存取。",
